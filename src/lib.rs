...
// ) -> Result<(), std::io::Error> {
//     while let Some(request) = rx.recv().await {
//         println!("Got new script:\n{}", request.script);

//         // 1) Check if robot connected/in normal mode, etc.
//         {
//             let st = ds.lock().unwrap();
//             if !st.connected || st.robot_state != 1 {
//                 eprintln!("Robot not in normal mode => fail script immediately.");
//                 let _ = request.result_tx.send(false);
//                 continue;
//             }
//             if st.goal_sender.is_some() {
...
//         // 2) Insert the new goal_sender
//         {
//             let mut st = ds.lock().unwrap();
//             st.goal_sender = Some(request.result_tx);
//         }

//         // 3) Connect to 30003 and write the script
//         let mut tcp = match TcpStream::connect(&ur_address).await {
//             Ok(s) => s,
//             Err(e) => {
//                 eprintln!("Failed to connect to {ur_address}: {e}");
...
//             }
//         };
//         if let Err(e) = tcp.write_all(request.script.as_bytes()).await {
...
//         println!("Script written. Now waiting for finish or cancel.");

//         // 4) Wait for either the script to complete or cancellation.
//         //    The real "script complete" event typically comes from the real-time
//         //    loop noticing program_state -> 1 again, which calls goal_sender.send(true/false).
//         //    So here, we just wait on that `goal_sender` to be used.
//         let script_finished = wait_for_script_finish(ds.clone());  // see below
//         tokio::select! {
//             // (A) The script finishes on its own
//             _ = script_finished => {
//                 println!("Script finished normally (true or false was already sent).");
//             }

//             // (B) The user cancels by sending to `cancel_rx`
//             _ = request.cancel_rx => {
//                 eprintln!("User requested CANCEL => sending Stop command to dashboard.");
//                 let (stop_tx, stop_rx) = oneshot::channel();
//                 dashboard_tx.send((DashboardCommand::Stop, stop_tx)).await
//                     .map_err(|_| std::io::Error::new(std::io::ErrorKind::Other, "Send Stop failed"))?;
//                 let dash_ok = stop_rx.await.unwrap_or(false);

//                 println!("Stop command => success? {dash_ok}");
//                 // We also want to make sure we mark the script as failed:
//                 let mut st = ds.lock().unwrap();
//                 if let Some(tx) = st.goal_sender.take() {
//                     let _ = tx.send(false);
//                 }
...
/// Waits until the script is done. In your code, the real-time loop sets
/// `goal_sender` = None => you might do something like a `Notify` or a oneshot
/// to signal that the script is done. This is just a placeholder to illustrate.
// async fn wait_for_script_finish(ds: Arc<Mutex<DriverState>>) {
//     // In reality, your realtime loop might do:
//     //   if program finished => goal_sender.send(true);
//     // so the script completes. The script_executor can wait for that in many ways.
//     // For demonstration, we just wait some indefinite time or have a watch channel, etc.

//     // In a real approach, you'd probably put a `tokio::sync::oneshot` or `Notify`
//     // in the driver state. Or you can do a busy loop checking .goal_sender 
//     // but that is less efficient. For example:
//     loop {
//         {
//             let st = ds.lock().unwrap();
//             if st.goal_sender.is_none() {
//                 // means the realtime loop used .take() => script finished
//                 break;
//             }
//         }
...
pub async fn start_driver(
    ...
    // Spawn script_executor
    {
        ...
        tokio::spawn(async move {
            if let Err(e) = script_executor(ds_clone, script_rx, rtde_clone, dash_clone).await {
